<template>
	<view class="container">
		<!-- 顶部导航栏 -->
		<!-- <view class="navbar">
			<text class="navbar-title">跟进列表</text>
		</view> -->
		...
		<scroll-view class="list-container" scroll-y="true" ref="listScroll" @scrolltolower="onReachBottom"
			...
			<view class="list-content">
				<view class="customer-item" v-for="(item, index) in followList" :key="index"
					@click="CustomerClick(item, index)">
					<view class="item-row">
						<!-- <text class="labelTitle">客户名：</text> -->
						<text class="contentTitle">{{ item.customer&&item.customer.cus_name }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">电话：</text> -->
						<text class="content">{{ item.follow_content }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">地址：</text> -->
						<text class="content">{{ item.follow_time }}</text>
					</view>
	...
	export default {
		...
		methods: {			
			updateData(data) {
				...
				const index = this.followList.findIndex(item => item.id === data.id);
				if (index !== -1) {
					// 使用 $set 触发响应式更新
					this.$set(this.followList, index, data);
			...
			loadList() {
				...
				this.$request("/api/FollowList", postdata)
					.then(res => {
						...
						// 合并数据
						this.followList = this.page === 1 ? res.data : [...this.followList, ...res.data];
			...
			CustomerClick(item, index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/follow/info?data=' + cusinfo
					});
				}, 500);
			}
		}
