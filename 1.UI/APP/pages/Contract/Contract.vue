<template>
	<view class="container">
		<!-- 顶部导航栏 -->
		<!-- <view class="navbar">
			<text class="navbar-title">合同列表</text>
		</view> -->
			...
			<view class="list-content">
				<view class="customer-item" v-for="(item, index) in dataList" :key="index"
					...
					<view class="item-row">
						...
						<text class="content">{{ item.Contract_name }}</text>
					</view>
					<view class="item-row">
	...
	export default {
		...
		methods: {
			...
			loadList() {
				...
				this.$request("/api/ContractList", postdata)
					.then(res => {
						console.log(res);
			...
			CustomerClick(item, index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/Contract/info?data=' + info
					});
				}, 500);
			}
		}
