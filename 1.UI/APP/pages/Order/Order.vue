	...
	<view class="container">
		...
		<scroll-view class="list-container" scroll-y="true" ref="listScroll" @scrolltolower="onReachBottom"
			...
			<view class="list-content">
				<view class="customer-item" v-for="(item, index) in orderList" :key="index"
					...
					<view class="item-row">
						...
						<text class="content">{{ item.Serialnumber }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">地址：</text> -->
						<text class="content">{{ item.total_amount }}</text>
					</view>
	...
	export default {
		...
		methods: {
			updateData(data) {
				...
				const index = this.orderList.findIndex(item => item.id === data.id);
				if (index !== -1) {
					// 使用 $set 触发响应式更新
			...
			loadList() {
				...
				this.$request("/api/OrderList", postdata)
					.then(res => {
						...
						// 合并数据
						this.orderList = this.page === 1 ? res.data : [...this.orderList, ...res.data];
			...
			CustomerClick(item, index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/Order/info?data=' + info
					});
				}, 500);
			}
		}
