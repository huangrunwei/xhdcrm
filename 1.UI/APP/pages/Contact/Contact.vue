<template>
	<view class="container">
		<!-- 顶部导航栏 -->
		<!-- <view class="navbar">
			<text class="navbar-title">联系人列表</text>
		</view> -->
		...
		<view class="search-container">
			...
			<view class="basic-search">
				<view class="search-input">
					...
					<input type="text" placeholder="输入联系人搜索" v-model="keyword"></input>
				</view>
		...
		<scroll-view class="list-container" scroll-y="true" ref="listScroll" @scrolltolower="onReachBottom"
			...
			<view class="list-content">
				<view class="customer-item" v-for="(item, index) in contactList" :key="index"
					@click="CustomerClick(item, index)">
					<view class="item-row">
						<!-- <text class="labelTitle">客户名：</text> -->
						<text class="contentTitle">{{ item.C_name }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">电话：</text> -->
						<text class="content">{{ item.C_tel }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">地址：</text> -->
						<text class="content">{{ item.customer&&item.customer.cus_name }}</text>
					</view>
            ...
            <text class="content">最后跟进：{{ item.lastFollowTime }}</text>
				...
				<!-- 空状态 -->
				<view class="empty-state" v-if="contactList.length === 0 && !loading">
	...
	export default {
		...
		methods: {
			updateData(data) {
				...
				const index = this.contactList.findIndex(item => item.id === data.id);
				if (index !== -1) {
					// 使用 $set 触发响应式更新
			...
			loadList() {
				...
				this.$request("/api/contactList", postdata)
					.then(res => {
						console.log(res);
						...
						// 合并数据
						this.contactList = this.page === 1 ? res.data : [...this.contactList, ...res.data];
			...
			CustomerClick(item, index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/Contact/info?data=' + cusinfo
					});
				}, 500);
			}
		}
