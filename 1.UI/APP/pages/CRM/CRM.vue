		...
		<view class="search-container">
			...
			<view class="basic-search">
				<view class="search-input">
					...
					<input type="text" placeholder="输入客户名称或电话搜索" v-model="keyword"></input>
				</view>
			...
			<view class="list-content">
				<view class="customer-item" v-for="(item, index) in customerList" :key="index"
					...
					<view class="item-row">
						...
						<text class="contentTitle">{{ item.cus_name }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">电话：</text> -->
						<text class="content">{{ item.cus_tel }}</text>
					</view>
					<view class="item-row">
						<!-- <text class="label">地址：</text> -->
						<text class="content">{{ item.cus_add }}</text>
					</view>
	...
	export default {
		...
		methods: {
			updateData(data) {
				...
				const index = this.customerList.findIndex(item => item.id === data.id);
				if (index !== -1) {
					// 使用 $set 触发响应式更新
					this.$set(this.customerList, index, data);
			...
			loadCustomerList() {
				...
				this.$request("/api/CustomerList", postdata)
					.then(res => {
						...
						// 合并数据
						this.customerList = this.page === 1 ? res.data : [...this.customerList, ...res.data];
			...
			CustomerClick(item, index) {
				...
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/CRM/CusInfo?cusinfo=' + cusinfo
					});
				}, 500);
			}
		}
