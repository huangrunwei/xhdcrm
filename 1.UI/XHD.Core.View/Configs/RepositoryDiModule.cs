...
namespace XHD.Core.View.Configs
{
    public static class RepositoryDiModule
    {
        ...
        {
            // 反射加载仓储程序集（与原Autofac一致，加载XHD.Core.Repository程序集）
            // 说明：Assembly.Load参数是「程序集名称（DLL文件名，不含.dll）」，与命名空间无关
            Assembly repositoryAssembly = Assembly.Load("XHD.Core.Repository");

            // 沿用原Autofac的筛选规则：类名以Repository结尾、非抽象类、非接口
            ...
            foreach (var repositoryType in repositoryTypes)
            {
                ...
                foreach (var @interface in implementedInterfaces)
                {
                    ...
                    //     foreach (var prop in repositoryType.GetProperties().Where(p => p.CanWrite))
                    //     {
