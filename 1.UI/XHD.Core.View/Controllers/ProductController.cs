...
namespace XHD.Core.View.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        private readonly SysLogExt<Product> logext = new SysLogExt<Product>();  //日志

        public ProductController(ILogger<ProductController> logger,IProductService Service, ISale_order_detailsService orderdetailsservice, ISys_logService LogService, IDBAuthService dBAuthService)
        {
            _logger = logger;
            _service = Service;
        ...
        public async Task<string> Grid(PageView<Product> model)
        {
            Expression<Func<Product, bool>> exp = a => 1 == 1;

            if (!string.IsNullOrWhiteSpace(Request.Query["T_type"]))
            {
                exp = exp.And(a => a.category_id == Request.Query["T_type"]);
            }
        ...
        public async Task<string> Save(Product model)
        {
            var result = 0;
            ...
            if (string.IsNullOrWhiteSpace(model.id))
            {
                ...
                //权限
                var authbtn = await _dBAuthService.GetAuth(User.FindFirst(ClaimTypes.Sid).Value, "Product|add");
            ...
            {
                //权限
                var authbtn = await _dBAuthService.GetAuth(User.FindFirst(ClaimTypes.Sid).Value, "Product|edit");

                if (authbtn)
                {
                    //日志
                    Expression<Func<Product, bool>> exp = a => a.id == model.id;
                    ...
                    if (content.Length > 0)
                    {
                        ...
                        logmodels.EventType = "[产品]修改";
                        logmodels.EventID = model.id;
                        logmodels.EventTitle = model.product_name;
        ...
        {
            //有订单，不能删除
            Expression<Func<Sale_order_details, bool>> exppro = a => a.product_id == id;

            var resultorder = await _orderdetailsservice.GridAsync(exppro, 1, 1);

            if (resultorder.count > 0)
            {
                return XHDResult.Error("类别下含有订单，不能删除！").ToString();
            }

            var result = 0;

            //权限
            var authbtn = await _dBAuthService.GetAuth(User.FindFirst(ClaimTypes.Sid).Value, "Product|del");

            if (authbtn)
            {
                //判断是否有数据
                Expression<Func<Product, bool>> exp = a => a.id == id;
                ...
                logmodels.id = UUIDNext.Uuid.NewSequential().ToString();
                logmodels.EventType = "[产品]删除";
                logmodels.EventID = id;
                logmodels.EventTitle = checkdata.data[0].product_name;
                logmodels.UserID = User.FindFirst(ClaimTypes.Sid).Value;
                logmodels.UserName = User.FindFirst(ClaimTypes.Name).Value;
                logmodels.IPStreet = HttpContext.Connection.RemoteIpAddress.ToString();
                logmodels.EventDate = DateTime.Now;
